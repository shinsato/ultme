...
function init(){
    ...
    //     console.log('TYPE TALLY::', element);
    //     var $e = $$(element);
    //     var val = $e.data('value');
    //     var $display = $$($e.find('.js-display'));
    //
    //     $e.tap(function(event){
    //         var $me = $$(this);
    ...
    // });
    tileTypeTally.tap(function(event){
        var $me = $$(event.target);
        saveTallyResponse(app,$me.data('rowid'));
        flashTile($me);
        updateTallyCount(app,$me.data('rowid'));
    });
    ...
    // overlay handlers
    $$('[toggle-overlay]').tap(function($event){
...
function loadTiles(app){
    ...
    app.db.transaction(
        function(tx){
            tx.executeSql('SELECT *,rowid FROM tile', [],
            function(tx,results){
